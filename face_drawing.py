...
class Turtle1ControllerNode:
    ...
    def __init__(self) -> None:
        ...
        rospy.Subscriber('/turtle1/pose', Pose, self.on_turtle1_pose_updated)
        self.set_pen_svc = rospy.ServiceProxy('turtle1/set_pen', SetPen)
    ...
    # the Pose topic of the turtlesim. 
...
#draws the smile
    def draw_circle3(self):
        #self.set_desired_point(5,1)
        self.set_pen_off(False)
        self.set_desired_point(3,3)        
        while not rospy.is_shutdown() and self.get_distance_to_goal() > 0.1:
            self.run_turtle_controller()
            self.sleep()                
#sets the starting point for the face
    def starting_point(self):
        ...
        while not rospy.is_shutdown() and self.get_distance_to_goal() > 0.1:
            self.run_turtle_controller()
            self.sleep()
#sets the starting pont for the smile
    def starting_point2(self):
        node.set_pen_off(True)
        node.set_desired_point(3,3)
        while not rospy.is_shutdown() and self.get_distance_to_goal() > 0.1:
            self.run_turtle_controller()
            self.sleep() 
#sets another starting point to make a happy smile instead of sad
    def starting_point3(self):
        node.set_pen_off(True)
        node.set_desired_point(5,3)
        while not rospy.is_shutdown() and self.get_distance_to_goal() > 0.1:
            self.run_turtle_controller()
            self.sleep() 
...
if __name__ == '__main__':
    try:
        ...
        # node.set_pen_off(False)
        # node.set_desired_point(5,10)
        # while not rospy.is_shutdown() and node.get_distance_to_goal() > 0.1:
        #     node.run_turtle_controller()
        # #     node.sleep()
        
        
        # # Create a desired location and then move until you are "close enough"
        # # Drives with the pen down and specifies the desired color
        # node.set_pen_off(False,r = 0, g = 255, b = 50)
        # node.set_desired_point(4,10)
        # while not rospy.is_shutdown() and node.get_distance_to_goal() > 0.1:
        #     node.run_turtle_controller()
        #     node.sleep()
